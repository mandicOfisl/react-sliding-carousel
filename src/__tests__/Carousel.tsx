...
const findDOMNodeWithinWrapper = (
...
describe('Slider', function() {
  ...
  const bootstrap = (
    ...
    component = mount<Partial<CarouselProps>>(
    ...
    totalChildren =
      children && children.length
        ? React.Children.count(componentInstance.props.children)
        : 0;
    ...
    describe('Default Props', () => {
      describe('values', () => {
        ...
        Object.keys(props).forEach((prop) => {
          it(`should have ${prop} as ${
            props[prop as keyof CarouselProps]
          }`, () => {
            expect(component.prop(prop)).toBeDefined();
            expect(component.prop(prop)).toEqual(
              props[prop as keyof CarouselProps]
            );
          });
        });
      ...
      describe('methods', () => {
        it('renderArrowPrev should return a button', () => {
          expect(
            componentInstance.props.renderArrowPrev!(jest.fn(), true, 'prev')
          ).toMatchSnapshot();
        });

        it('renderArrowNext should return a button', () => {
          expect(
            componentInstance.props.renderArrowNext!(jest.fn(), true, 'next')
          ).toMatchSnapshot();
        });

        it('renderIndicator should return a list item', () => {
          expect(
            componentInstance.props.renderIndicator!(
              jest.fn(),
              true,
              0,
              'slide'
        ...
        it('renderItem should pass through the item', () => {
          expect(
            componentInstance.props.renderItem!(<div>item</div>)
          ).toMatchSnapshot();
        });
        ...
        it('statusFormatter should return a string', () => {
          expect(componentInstance.props.statusFormatter!(1, 3)).toEqual(
            '1 of 3'
          );
        });
      });
  ...
  describe('getInitialImage', () => {
    it('Returns the first image within the declared selected item', () => {
      ...
      expect(
        initialImage.src.endsWith(expectedMatchingImageComponent.props.src)
      ).toEqual(true);
    });
  });
  ...
  describe('Moving', () => {
    ...
    it('should update the position of the Carousel if selectedItem is changed', () => {
      findDOMNodeWithinWrapper(
        component,
        componentInstance.itemsRef[2]
      ).simulate('click');
      expect(componentInstance.state.selectedItem).toBe(2);

      findDOMNodeWithinWrapper(
        component,
        componentInstance.itemsRef[3]
      ).simulate('click');
      expect(componentInstance.state.selectedItem).toBe(3);
    });
  });
  ...
  const findDOMNodeByClass = (instance: any, classNames: string) =>
    (ReactDOM.findDOMNode(instance)! as HTMLElement).querySelectorAll(
      classNames
    );
  ...
  describe('Swiping', () => {
    describe('onSwipeStart', () => {
      ...
      it('should call onSwipeStart callback', () => {
        const onSwipeStartFunction = jest.fn();
    ...
    describe('onSwipeMove', () => {
      ...
      it('should call onSwipeMove callback', () => {
        const onSwipeMoveFunction = jest.fn();
    ...
    describe('onSwipeEnd', () => {
      ...
      it('should call onSwipeEnd callback', () => {
        const onSwipeEndFunction = jest.fn();

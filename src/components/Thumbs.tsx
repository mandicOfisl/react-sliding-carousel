...
): e is React.KeyboardEvent => e.hasOwnProperty('key');
...
export default class Thumbs extends Component<Props, State> {
  ...
  onSwipeMove = (delta: { x: number; y: number }) => {
    ...
    const currentPosition =
      -(this.state.firstItem * 100) / this.state.visibleItems;
    ...
    if (this.itemsListRef) {
      [
        ...
        this.itemsListRef!.style[prop as any] = CSSTranslate(
          position,
          '%',
          this.props.axis
        );
  ...
  moveTo = (position: number) => {
    ...
    position =
      position >= this.state.lastPosition ? this.state.lastPosition : position;
  ...
  renderItems() {
    return this.props.children.map((img, index) => {
      ...
      const thumbProps = {
        ...
        onKeyDown: this.handleClickItem.bind(
          this,
          index,
          this.props.children[index]
        ),
